...
def set_global(args):
    ...
    src_include_dir = iopc.getBaseRootFile("usr/include/nettle")
...
def MAIN_EXTRACT(args):
    ...
    ops.copyto(ops.path_join(src_usr_lib_dir, "libhogweed.so.4.3"), dst_usr_lib_dir)
    ops.ln(dst_usr_lib_dir, "libhogweed.so.4.3", "libhogweed.so.4")
    ops.ln(dst_usr_lib_dir, "libhogweed.so.4.3", "libhogweed.so")

    ops.copyto(ops.path_join(src_usr_lib_dir, "libnettle.so.6.3"), dst_usr_lib_dir)
    ops.ln(dst_usr_lib_dir, "libnettle.so.6.3", "libnettle.so.6")
    ops.ln(dst_usr_lib_dir, "libnettle.so.6.3", "libnettle.so")
    return True
